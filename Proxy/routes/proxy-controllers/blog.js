...
exports.listAllBlogsCategoriesTags = (req, res) => {
    ...
    Shop.findOne({shopify_domain: shopName}).exec((err, shop) => {
        ...
        .skip(skip)
        .limit(limit)
        .select('_id title coverMedia slug product_imgurl product_summary mdesc autoGenerated categories tags postedBy createdAt updatedAt')
        .exec((err, data) => {
            if (err) {
                return res.json({
                    error: errorHandler(err)
            ...
            .exec((err, blog) => {
                if (err) {
                 ...
                 User.findOne({ _id: blog.postedBy })
                    ...
                    res.send(blogSlug({blog, shop, user}));
                    });
            });
        });
