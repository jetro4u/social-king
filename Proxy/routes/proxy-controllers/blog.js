...
exports.read = (req, res) => {
    ...
    Blog.findOne({ slug })
        ...
        .select('_id title hidden cover_photo html body selectedProducts product_rating total_ratings autoGenerated slug mtitle mdesc tags postedBy trackingID createdAt updatedAt')
        .exec((err, data) => {
            if (err) {
                return res.json({
                    error: errorHandler(err)
            ...
            let shop = {};
            shop.shopify_domain = req.query.shop
            res.send(blogSlug({blog: data, shop }));
        });
};
