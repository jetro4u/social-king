    ...
    User.findOne({ username }).exec((err, userFromDB) => {
        ...
        if(userFromDB.email == 'alephmarketingpros@gmail.com'){
            ...
            Blog.find({ reviewedBy: userId })
                ...
                .limit(1000)
                .select('_id popUser title slug autoGenerated product_imgurl mdesc excerpt categories tags postedBy createdAt updatedAt')
                .exec((err, data) => {
                    if (err) {
                        return res.status(400).json({
            ...
            Blog.find({ postedBy: userId })
                ...
                .limit(10)
                .select('_id title slug autoGenerated product_imgurl mdesc excerpt categories tags postedBy createdAt updatedAt')
                .exec((err, data) => {
                    if (err) {
                        return res.status(400).json({
                            error: errorHandler(err)
