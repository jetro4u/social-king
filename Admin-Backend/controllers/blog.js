...
exports.read = (req, res) => {
    console.log('req.query in read blog function', req.query);
    ...
    Shop.findOne({shopify_domain: req.query.shop}).exec((err, shop) => {
        ...
        Blog.findOne({ slug })
            ...
            .select('_id title selectedProducts body total_ratings slug mtitle mdesc tags postedBy trackingID createdAt updatedAt')
            .exec((err, data) => {
                if (err) {
                    return res.json({
                        error: errorHandler(err)
                ...
                data.shopify_domain = req.query.shop;
                res.json(data);
            });
    });
};
